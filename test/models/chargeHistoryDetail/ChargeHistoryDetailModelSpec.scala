  ...
  "ChargeHistoryDetail" should {
    "read from json" when {
      "the json is complete" in {
        Json.fromJson[ChargeHistoryDetailModel](testValidChargeHistoryModelJson) shouldBe JsSuccess(chargeHistoryDetail)
      }
    }
    "write to json" when {
      "the model is complete" in {
        Json.toJson(chargeHistoryDetail) shouldBe testValidChargeHistoryModelJson
      }
    }
  }
}
