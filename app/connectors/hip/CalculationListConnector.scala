...
import connectors.hip.httpParsers.CalculationListHttpParser.{CalculationListReads, HttpGetResult}
...
class CalculationListConnector @Inject()(val http: HttpClientV2, val appConfig: MicroserviceAppConfig) extends Logging {
  ...
  def getCalculationList(nino: String, taxYear: String)
    ...
    logger.debug(
      s"Calling GET $url \nHeaders: $headerCarrier \nAuth Headers: ${appConfig.getHIPHeaders(API_1404_CONFIG_NAME, headerCarrier.requestId.map(_.value))}" +
        s" \nIs1404GetCalcApiEnabledInHip: ${if (isGetCalcApiEnabledInHip) "YES" else "NO"}"
    )

    http.get(url"$url")
      .setHeader(
        appConfig.getHIPHeaders(API_1404_CONFIG_NAME, headerCarrier.requestId.map(_.value)): _*
      )
